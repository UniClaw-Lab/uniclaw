"""``BoardProofClient`` — the Python adapter for BoardProof's HTTP
...
claim), :class:`BoardProofVerifyError` is raised. The caller can opt
...
class BoardProofClient:
    """Idiomatic Python client for the BoardProof HTTP proposal API.
    ...
    Args:
        ...
        bearer_token: 32-byte bearer token as a 64-char hex string.
            ...
            Required by ``boardproof-host`` started with
    ...
    def get_receipt(self, content_id: str) -> dict[str, Any]:
        ...
        if not isinstance(result, dict):
            raise BoardProofError(200, "malformed_response", "receipt JSON was not an object")
        return result
    ...
    def _post(self, path: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            raise BoardProofError(0, "fetch_failed", f"POST {url}: {e.reason}") from e
    ...
    def _get(self, url: str) -> str:
        ...
        try:
            ...
            raise BoardProofError(0, "fetch_failed", f"GET {url}: {e.reason}") from e

    def _parse_http_error(self, e: urllib.error.HTTPError) -> BoardProofError:
        """Map a urllib HTTPError to a typed BoardProofError, parsing the
        server's ``{error, detail}`` body when possible."""
        status = e.code
        try:
            body = e.read().decode("utf-8")
        except Exception:  # pragma: no cover — best effort
            return BoardProofError(status, "non_text_response", "<unreadable response body>")
        try:
            parsed: Any = json.loads(body)
        except json.JSONDecodeError:
            return BoardProofError(status, "non_json_response", body[:200])
        if isinstance(parsed, dict) and isinstance(parsed.get("error"), str) and isinstance(parsed.get("detail"), str):
            return BoardProofError(status, parsed["error"], parsed["detail"])
        return BoardProofError(status, "unknown", str(parsed)[:200])
    ...
    def _build_decision(self, resp: dict[str, Any]) -> Decision:
        ...
        if not isinstance(decision_str, str):
            raise BoardProofError(200, "malformed_response", "decision must be a string")
        receipt_url = resp["receipt_url"]
        if not isinstance(receipt_url, str):
            raise BoardProofError(200, "malformed_response", "receipt_url must be a string")
