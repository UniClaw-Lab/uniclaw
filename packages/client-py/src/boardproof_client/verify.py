...
from .errors import BoardProofError
...
def verify_receipt_url(url: str, *, timeout: float = 10.0) -> VerifyResult:
    ...
    :class:`BoardProofError` if the GET itself fails (network error /
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:  # noqa: S310 (trusted URL by design)
            if resp.status < 200 or resp.status >= 300:
                raise BoardProofError(resp.status, "fetch_failed", f"GET {url} → HTTP {resp.status}")
            text = resp.read().decode("utf-8")
    except urllib.error.HTTPError as e:
        raise BoardProofError(e.code, "fetch_failed", f"GET {url} → HTTP {e.code}") from e
