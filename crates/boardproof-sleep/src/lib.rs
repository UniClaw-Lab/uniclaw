//! Sleep stages for BoardProof — Light, REM, and Deep Sleep.
//!
...
//!   wraps any `boardproof_store::ReceiptLog` and walks its `verify_chain()`.
//!
...
//! ## Where this fits
//!
//! `boardproof-sleep` is the **Spine** layer's background-task surface
//! (master plan §9). The kernel consumes the [`LightSleepReport`] /
//! [`DeepSleepReport`] this crate produces and turns each into a signed
//! audit receipt — see `boardproof-kernel`'s `KernelEvent::RunLightSleep`
...
/// A built-in [`Walkable`] that wraps any `boardproof_store::ReceiptLog`
/// and runs `verify_chain()` as its integrity walk.
///
/// Lives in `boardproof-sleep` so the kernel doesn't need a direct
...
#[derive(Debug)]
pub struct ReceiptLogWalker<'a, L: boardproof_store::ReceiptLog + ?Sized> {
...
impl<'a, L: boardproof_store::ReceiptLog + ?Sized> ReceiptLogWalker<'a, L> {
...
impl<L: boardproof_store::ReceiptLog + ?Sized> Walkable for ReceiptLogWalker<'_, L> {
    fn name(&self) -> &str {
        &self.name
    }
...
mod tests {
    ...
    fn receipt_log_walker_reports_failure_when_walk_fails() {
        ...
        // `boardproof-store`'s own tests (verify_chain catches body mutation
