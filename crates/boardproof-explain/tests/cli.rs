//! Integration tests: drive the actual `boardproof-explain` binary as a
...
const EXPLAIN_BIN: &str = env!("CARGO_BIN_EXE_boardproof-explain");
...
fn run(json: &str, extra: &[&str]) -> std::process::Output {
    let mut child = Command::new(EXPLAIN_BIN)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn boardproof-explain");
    child
        .stdin
        .take()
        .expect("stdin piped")
...
fn json_mode_emits_parseable_json() {
    ...
    assert!(
        parsed["canonical_url"]
            .as_str()
            .unwrap()
            .starts_with("boardproof://receipt/")
    );
}
