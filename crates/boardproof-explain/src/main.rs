//! `boardproof-explain` — print a human-readable decision tree for any receipt.
//!
...
use boardproof_receipt::Receipt;
use clap::Parser;
...
fn main() -> Result<()> {
    ...
    let exp = boardproof_explain::explain(&receipt);

    let out = if cli.json {
        boardproof_explain::render_json(&exp)
    } else {
        boardproof_explain::render_text(&exp)
    };
    println!("{out}");
    ...
    if matches!(
        exp.signature,
        boardproof_explain::SignatureStatus::Failed(_)
    ) {
        std::process::exit(2);
    }
    Ok(())
