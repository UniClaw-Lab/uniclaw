//! Cold receipt explainer for BoardProof.
//!
...
//! `boardproof-explain` on any receipt without trusting the runtime that
//! produced it.

mod render;

use boardproof_budget::BudgetError;
use boardproof_receipt::{Action, Decision, MerkleLeaf, Receipt, RuleRef, VerifyError, crypto};
...
pub struct Explanation {
    ...
    /// Canonical addressable URL — `boardproof://receipt/<id>`.
...
pub fn explain(receipt: &Receipt) -> Explanation {
    ...
    Explanation {
        canonical_url: format!("boardproof://receipt/{receipt_id}"),
        receipt_id,
...
mod tests {
    ...
    fn explain_full_pipeline_on_signed_allowed_receipt() {
        ...
        assert!(exp.canonical_url.starts_with("boardproof://receipt/"));
