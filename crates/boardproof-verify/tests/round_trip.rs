...
//! `boardproof-verify` binary as a subprocess. This is the milestone exit
...
const VERIFY_BIN: &str = env!("CARGO_BIN_EXE_boardproof-verify");
...
fn run_verify(json: &str, extra: &[&str]) -> std::process::Output {
    let mut child = Command::new(VERIFY_BIN)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("spawn boardproof-verify");
    child
        .stdin
        .take()
        .expect("stdin piped")
...
fn wrong_issuer_fails() {
    ...
    receipt.issuer = boardproof_receipt::PublicKey(other.verifying_key().to_bytes());
    let json = serde_json::to_string(&receipt).unwrap();
    let out = run_verify(&json, &[]);
    assert!(!out.status.success());
}
