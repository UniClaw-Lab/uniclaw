...
impl Signer for Ed25519Signer {
    ...
    fn public_key(&self) -> boardproof_receipt::PublicKey {
        boardproof_receipt::PublicKey(self.0.verifying_key().to_bytes())
    }
}
...
fn deep_sleep_pass_walks_real_receipt_log_and_mints_signed_receipt() {
    ...
    let issuer = boardproof_receipt::PublicKey(key.verifying_key().to_bytes());
    let mut audit = InMemoryReceiptLog::new(issuer);
    let mut prev = Digest([0u8; 32]);
    for i in 0..8 {
        let mut body = ReceiptBody {
            schema_version: boardproof_receipt::RECEIPT_FORMAT_VERSION,
            ...
            key_id: None,
            merkle_leaf: boardproof_receipt::MerkleLeaf {
                sequence: i,
        ...
        let r = boardproof_receipt::crypto::sign(body, &key);
...
fn empty_deep_sleep_pass_is_a_meaningful_audit_event() {
    ...
    let _: Option<ReceiptLogWalker<'_, boardproof_store::InMemoryReceiptLog>> = None;
}
...
fn tool_execution_with_redaction_uses_redacted_hash_and_emits_applied_edges() {
    ...
    let report = boardproof_receipt::RedactionReport {
        redacted_output_hash: redacted_hash,
        matches: vec![boardproof_receipt::RuleMatch {
