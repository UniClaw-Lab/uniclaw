...
use boardproof_budget::{CapabilityLease, ResourceUse};
use boardproof_receipt::{Action, Decision, ProvenanceEdge, Receipt, RedactionReport, RuleRef};
use boardproof_sleep::{DeepSleepReport, LightSleepReport};
use boardproof_tools::{ToolError, ToolOutput};
...
/// (typically via [`boardproof_tools::ToolHost`]), takes the
...
pub struct ToolExecution {
    ...
    /// 3. Populates [`boardproof_receipt::ReceiptBody::redactor_stack_hash`]
...
pub enum KernelEvent {
    ...
    /// happens in `boardproof-sleep::run_light_sleep`; the kernel only signs
    /// the audit receipt.
    ...
    /// `boardproof-sleep::run_deep_sleep`; the kernel only signs the audit
