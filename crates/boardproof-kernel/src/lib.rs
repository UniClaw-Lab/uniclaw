//! BoardProof kernel — the trusted runtime core.
//!
...
// on `boardproof-sleep` to construct `KernelEvent::RunLightSleep` /
// `RunDeepSleep`.
pub use boardproof_sleep::{
...
// on `boardproof-tools` to construct `KernelEvent::RecordToolExecution`.
pub use boardproof_tools::{
