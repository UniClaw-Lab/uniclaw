...
use boardproof_approval::ApprovalDecision;
use boardproof_budget::BudgetError;
use boardproof_constitution::Constitution;
use boardproof_receipt::{
    Action, Decision, Digest, MerkleLeaf, ProvenanceEdge, RECEIPT_FORMAT_VERSION, Receipt,
    ReceiptBody, RuleRef, crypto,
};
use boardproof_sleep::{DeepSleepReport, LightSleepReport};
...
mod tests {
    ...
    use boardproof_budget::{Budget, CapabilityLease, LeaseId, ResourceUse};
    use boardproof_constitution::{
        EmptyConstitution, InMemoryConstitution, MatchClause, Rule, RuleVerdict,
    };
    use boardproof_receipt::{Action, Decision, Digest, Receipt, ReceiptBody};
    ...
    impl Signer for StubSigner {
        fn sign(&self, body: ReceiptBody) -> Receipt {
            Receipt {
                ...
                body,
                issuer: boardproof_receipt::PublicKey([0xAA; 32]),
                signature: boardproof_receipt::Signature([0xBB; 64]),
            }
        }

        fn public_key(&self) -> boardproof_receipt::PublicKey {
            boardproof_receipt::PublicKey([0xAA; 32])
        }
    }
    ...
    use boardproof_sleep::{CleanerPass, CleanupError, CleanupReport, LightSleepReport};
    ...
    use boardproof_sleep::{DeepSleepReport, WalkError, WalkReport, WalkerPass};
