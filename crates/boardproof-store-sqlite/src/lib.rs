//! SQLite-backed `ReceiptLog` for BoardProof.
//!
...
//! this in place, `boardproof-host` can serve receipts that outlive a
...
use boardproof_receipt::{Digest, PublicKey, RECEIPT_FORMAT_VERSION, Receipt, crypto};
use boardproof_store::{AppendError, ReceiptLog, VerifyChainError};
...
/// so the struct itself is `Sync`; the `boardproof-host` runtime needs that
...
impl SqliteReceiptLog {
    ...
    pub fn peek_issuer(path: impl AsRef<Path>) -> Result<Option<PublicKey>, OpenError> {
        ...
        // initialized as a boardproof-store-sqlite log). Treat that as None.
...
mod tests {
    ...
    use boardproof_receipt::{Action, Decision, MerkleLeaf, ReceiptBody, Signature};
