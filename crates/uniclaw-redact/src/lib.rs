//! Output sanitization for Uniclaw tool outputs.
//!
...
//! forever.
//!
//! `uniclaw-redact` runs operator-configured pattern matching
...
//! [`uniclaw_receipt::RedactionReport`] that the kernel records on
...
//!   the caller plus a [`uniclaw_receipt::RedactionReport`] for
//!   the kernel.
...
//!   resulting [`uniclaw_receipt::RedactionReport`] as audit
...
//!   A future step ties into `uniclaw-secrets` to scan for the
...
// Re-export the audit-data types from `uniclaw-receipt` so callers
// using the redactor have a single import path.
pub use uniclaw_receipt::{RedactionReport, RuleMatch};
