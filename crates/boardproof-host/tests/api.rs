...
use boardproof_host::api::{ApiState, AuthConfig, api_router};
use boardproof_host::clock::SystemClock;
use boardproof_host::router;
use boardproof_host::signer::Ed25519Signer;
use boardproof_kernel::{Kernel, Signer};
use boardproof_receipt::{Receipt, crypto};
...
async fn allowed_proposal_returns_signed_receipt_at_seq_0() {
    ...
    assert_eq!(receipt.body.decision, boardproof_receipt::Decision::Allowed);
}
...
fn temp_db_path(suffix: &str) -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!("boardproof-host-step26-{pid}-{n}-{suffix}.db"))
}
...
fn build_app_with_sqlite(log: SqliteReceiptLog) -> Router {
    ...
    let kernel = boardproof_host::api::build_kernel_from_log(&log, signer, constitution);
    let log = Arc::new(RwLock::new(log));
    let state = ApiState::new(kernel, log.clone());
    router(log).merge(api_router(state, AuthConfig::insecure()))
}

fn test_issuer() -> boardproof_receipt::PublicKey {
...
async fn sqlite_reopen_with_wrong_issuer_is_caught_by_peek_issuer() {
    ...
    let wrong_issuer = boardproof_receipt::PublicKey([0u8; 32]);
