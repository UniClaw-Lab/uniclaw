...
use boardproof_store::{AppendError, InMemoryReceiptLog, ReceiptLog};
...
fn pubkey(k: &SigningKey) -> boardproof_receipt::PublicKey {
    boardproof_receipt::PublicKey(k.verifying_key().to_bytes())
}

/// Simulate the kernel's leaf-hash computation so we can mint chained
/// receipts directly in tests without depending on `boardproof-kernel`.
