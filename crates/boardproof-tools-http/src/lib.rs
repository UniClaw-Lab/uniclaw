//! HTTP fetch tool for BoardProof — capability-checked GET with SSRF
//! defense, response-size bounds, and configurable timeout.
//!
//! This is the **first real `Tool` implementation** in the workspace
//! (Phase 3 step 14). It validates the [`boardproof_tools::Capability`]
...
//!   `User-Agent: boardproof-tools-http/<version>`.
//!
//! ## Defenses
//!
//! - **Capability allowlist.** Each [`HttpFetchTool`] is constructed
//!   with a list of [`boardproof_tools::GlobPattern`]s; requests whose
//!   host doesn't match any pattern fail with
//!   [`boardproof_tools::ToolError::CapabilityDenied`] without opening a
//!   socket.
...
//!   with [`boardproof_tools::ToolError::Failed`] — the body is **not**
//!   returned partially.
...
//!   from `boardproof-tools` before any network activity.
//!
...
use boardproof_secrets::SecretBroker;
use boardproof_tools::{
...
mod tests {
    ...
    fn auth_set_with_unknown_secret_fails_closed_before_network() {
        ...
        let broker = Arc::new(boardproof_secrets::InMemorySecretBroker::new());
