...
- `@boardproof/verifier` (step 20a) — TS verifier.
- `@boardproof/client` (step 22) — TS HTTP client + verify-by-default.
- `boardproof-client` (step 24) — Python verifier + HTTP client.

They were *built* but not *published*. `npm install @boardproof/client` did not work — the package wasn't on the registry. `pip install boardproof-client` did not work either. Threshold 1 ("a TypeScript developer can `npm install` a verifier and validate a BoardProof receipt minted on a Rust kernel") was conceptually closed since step 24 but **literally** still needed the registry push.
...
| Blocker | Where | Fix |
|---|---|---|
| `@boardproof/client` depended on `"file:../verifier-ts"` | `packages/client-ts/package.json` | Changed to `"^0.1.0"`; root npm workspaces resolves it locally for dev. |
...
- `npm install @boardproof/client` — works.
- `npm install @boardproof/verifier` — works.
- `pip install boardproof-client` — works.
...
The TS monorepo had a *development-time* convenience that was a *publish-time* blocker: `@boardproof/client` used `"@boardproof/verifier": "file:../verifier-ts"` so `npm install` from inside `packages/client-ts` worked during development. But `npm publish` records that string literally — anyone installing `@boardproof/client@0.1.0` from npm would get a manifest pointing at a directory that doesn't exist on their machine.
...
and change the client's dependency to `"@boardproof/verifier": "^0.1.0"`. Now:

- **Local dev** (`npm install` from repo root) — npm sees both workspaces, symlinks `node_modules/@boardproof/verifier → packages/verifier-ts`, and `@boardproof/client` resolves its dep to the *local* source. Tests work against the live verifier.
- **Publish** (`npm publish -w @boardproof/client`) — npm packs the client with the literal `^0.1.0` dependency. Anyone installing it pulls `@boardproof/verifier@0.1.0` from the npm registry.

Same `dist/` artifacts, same tests, two different resolution paths. The workspace declaration is the only structural change.

For Python it was simpler — `boardproof-client` had no inter-package dependency to manage. The remaining gap was just LICENSE files in the sdist + wheel, fixed by copying both LICENSE-MIT and LICENSE-APACHE into `packages/client-py/`. Setuptools 68+ auto-discovers `LICENSE*` at the package root.
...
1. setuptools reads `pyproject.toml`, builds a wheel (`boardproof_client-0.1.0-py3-none-any.whl`) and an sdist (`boardproof_client-0.1.0.tar.gz`).
...
- Confirm `@boardproof` npm org exists and the operator has publish rights.
...
# @boardproof/verifier@0.1.0 to https://www.npmjs.com/package/@boardproof/verifier

cd ../client-ts
npm publish
# Same flow. After this, npm install @boardproof/client resolves
# @boardproof/verifier from the registry (not the local workspace).
```
...
npm install @boardproof/client@0.1.0
node -e "
  const { BoardProofClient } = require('@boardproof/client');
  console.log('installed; BoardProofClient =', typeof BoardProofClient);
"
```
...
- Confirm `boardproof-client` is reserved on PyPI by the operator's account (https://pypi.org/manage/account/projects/).
...
pip install boardproof-client==0.1.0
python -c "from boardproof_client import BoardProofClient; print('ok')"
```
...
For the TS client, the `@boardproof/verifier` dependency uses `^0.1.0` — that resolves to any `0.1.x`. If the verifier ever publishes a `0.2.0` (additive but pre-1.0 semver still treats minor bumps as breaking), the client manifest needs a manual bump to `"^0.2.0"`.
...
TS verifier:  42 passed (npm test --workspace @boardproof/verifier)
TS client:    52 passed (BOARDPROOF_INTEGRATION=1 npm test --workspace @boardproof/client)
Python:       84 passed (BOARDPROOF_INTEGRATION=1 pytest in packages/client-py)
...
```
@boardproof/verifier  →  14.9 kB packed, 47.3 kB unpacked, 29 files.
@boardproof/client    →  15.1 kB packed, 48.9 kB unpacked, 20 files.
```
...
`packages/client-ts/tests/bench.mjs` had been broken since step 25 (PR #33): the bench spawns the host without `--bearer-token-hex` and without `--insecure-no-auth`, so step 25's safe-default check (proposal mode refuses to start without an explicit auth choice) made the bench error out at startup. The Python bench was updated in step 25 but the TS bench wasn't. Fixed in this PR by adding `--bearer-token-hex`, `bearerToken` to both `BoardProofClient` instances, and an `Authorization: Bearer` header on the raw-fetch baseline. This is a one-file dev-tools fix; no library behavior changes.
