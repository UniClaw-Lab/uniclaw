# Phase 3.5 Step 22 — `@boardproof/client` TypeScript SDK

> **Phase:** 3.5 — Receipt-format hardening + adoption-foundations
> **PR:** _this PR_
> **New top-level dir:** `packages/client-ts/` (second JS/TS package; first was `@boardproof/verifier` in step 20a)
...
```ts
import { BoardProofClient } from "@boardproof/client";

const client = new BoardProofClient({ baseUrl: "http://127.0.0.1:8787" });
...
┌─────────────────────────────┐    POST /v1/proposals
│  TypeScript runtime         │ ───────────────────────►  ┌─────────────────────┐
│  (OpenClaw / NemoClaw / …)  │                            │ boardproof-host        │
│                             │ ◄───────────────────────   │ --constitution=…    │
│  @boardproof/client            │       signed receipt       │ (step 21)           │
│       │                     │                            └──────────┬──────────┘
│       │ verify-by-default   │                                       │
│       ▼                     │      GET /receipts/<hash>             │
│  @boardproof/verifier          │ ───────────────────────────────────►  │
...
The three packages compose: `boardproof-host` mints over HTTP, `@boardproof/verifier` does the cryptographic check, `@boardproof/client` orchestrates both and gives callers a clean idiomatic API. Together they close threshold 3's first concrete deliverable — *a real adapter that an OpenClaw-style runtime would install*.

## What problem does it solve technically?

Three problems.

### 1. "How do I integrate BoardProof into my TS runtime?"

Before step 22, the answer was *"call `POST /v1/proposals` yourself, parse the JSON, look up `@boardproof/verifier`, etc."* That's twelve lines of glue every integrator has to write — and the failure modes (forgetting to verify, mixing snake_case/camelCase, not handling pending) are silent.
...
4. **Reconstruct canonical body bytes** via `@boardproof/verifier`'s JCS port.
...
If any check fails, `BoardProofVerifyError` is thrown before the caller sees the decision. The integration test exercises this: it intercepts the GET response, mutates one byte of the body, and confirms the client rejects it (`verify-by-default catches a tampered receipt`).
...
`@boardproof/client` handles this two ways:
...
- **`src/client.ts`** — `BoardProofClient`. Wraps fetch, converts camelCase ↔ snake_case at the boundary, builds the Decision union, and runs the verifier when `verify` is true. Three private mutex'd methods (`#postProposal`, `#postResolve`, `#parseError`), three public methods, one constructor.
- **`src/error.ts`** — `BoardProofError` (HTTP-status-mapped) and `BoardProofVerifyError` (signature/hash failures). Both are simple `Error` subclasses with typed fields.
...
- **Integrate from any TS runtime:**
  ```bash
  npm install @boardproof/client
  ```
  ```ts
  import { BoardProofClient } from "@boardproof/client";
  const c = new BoardProofClient({ baseUrl: process.env.BOARDPROOF_HOST! });
  const d = await c.evaluate({ kind: "http.fetch", target: "...", inputHash: "..." });
  ```
- **Skip verification** (faster) when you trust the network path completely: `new BoardProofClient({ baseUrl, verifyByDefault: false })`.
- **Inject a custom fetch** for auth headers / mTLS / custom user agents:
  ```ts
  new BoardProofClient({ baseUrl, fetch: (url, init) =>
...
- **24 tests pass** across two files:
  ...
  - `tests/integration.test.ts` (7 tests, skipped without `BOARDPROOF_INTEGRATION=1`) — drives a live `boardproof-host` subprocess through allowed/denied/pending→approved/pending→denied. Includes a **tamper test** that intercepts the GET response, mutates the `decision` field, and confirms `verify-by-default` rejects it.
...
- No source borrowed from any other claw. The discriminated-union Decision shape and verify-by-default discipline are original BoardProof idioms.
...
- **Authentication.** The wire format accepts no auth headers today. When `boardproof-host` adds bearer-token / mTLS, callers will inject auth via the `fetch` option without any client change.
...
- **`npm publish`.** Operations task — credentials, release process. The package code, tests, and README are in this PR; publishing is one command (`npm publish --access=public`) once the org reserves `@boardproof` on npm.
...
- `dist/` after `npm run build`: 6 modules + maps + .d.ts; transitive install with `@boardproof/verifier` is ~250 KB.
- See the bench file above for the latency breakdown.

## In summary

Step 22 makes "integrate BoardProof into a TypeScript runtime" a one-line operation. Combined with step 21 (server) and step 20a (verifier), the receipt-as-protocol claim now has an end-to-end TypeScript story:
...
The receipt was portable. The receipt was demonstrable. The receipt was integrable. **The receipt is now installable** — `npm install @boardproof/client`.
