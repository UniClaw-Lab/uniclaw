...
> A short, plain-English introduction. After reading this, you should know **what BoardProof is**, **what it does**, **who it's for**, and **why it's different**.

## In one sentence

**BoardProof is an AI agent runtime that makes every action it takes provable, so that anyone — not just the operator — can verify what the agent did.**

## The problem BoardProof solves
...
These users need a record that **(a)** cannot be quietly edited and **(b)** can be checked by an outsider who does not trust the operator. That record is what BoardProof produces, and it calls each one a **receipt**.

## What is a "receipt"?

A receipt is a small, signed file. Every time the agent does something — *anything* — BoardProof produces a receipt that captures:
...
The published goal: every receipt will eventually be reachable at a public URL like `https://boardproof.dev/receipts/<hash>`, so that an auditor on the other side of the world can look at it without trusting anyone in the loop.

## What can you do with BoardProof?
...
5. **Verify receipts** with a tiny standalone tool (`boardproof-verify`) that has no internet access and no database. You can run it on an offline laptop.
6. **Explain receipts** in plain English with `boardproof-explain` — useful when your auditor wants a human-readable version.
...
## Who is BoardProof for?

BoardProof is **not trying to be the agent for everyone**. It is built specifically for users who care about **proof** more than they care about convenience. In rough priority order:

| Audience | Why they need BoardProof |
|---|---|
...
| **Mobile-first users** who want privacy | BoardProof plans an Android-native profile where the model runs *on your phone*, with the phone's secure hardware verifying sensor inputs. |
...
If you just want a chatbot, BoardProof is overkill. If you need to prove what your agent did to a person who does not trust you, BoardProof is the point.

## What can BoardProof do? (skills)

This is the short list of "skills" that make BoardProof different from other agent runtimes:
...
## How do you use BoardProof?

BoardProof is **early**. Today you:

- Clone the repo from `https://github.com/UniClaw-Lab/boardproof`.
- Run `cargo build --workspace`.
- Use the libraries (`boardproof-kernel`, `boardproof-store`, `boardproof-constitution`, …) from a Rust program.
- Use the standalone binaries `boardproof-verify` and `boardproof-explain` from the command line.

Soon (Phase 2):

- Public-URL receipt hosting at `boardproof.dev`. You'll be able to share a URL with an auditor and they will verify it in their browser, with no BoardProof account needed.
...
There is a small family of "claw" projects in the AI agent space (OpenClaw, ZeroClaw, NanoClaw, OpenFang, NemoClaw, PicoClaw, NullClaw, IronClaw, LocalClaw — nine in total). They all approach AI agents differently. **BoardProof** is the one that picked **verifiability** as its central promise. The name signals "one consistent claw" with a single, focused thing it tries to be best at: **proof you can actually use**.

For a head-to-head with the most popular of those projects, see [BoardProof vs OpenClaw](02-boardproof-vs-openclaw.md).

## What BoardProof is NOT

Equally important — these are things BoardProof deliberately does **not** try to be:
