# The BoardProof Roadmap (8 phases, plain English)
...
The order matters. Many AI agent projects build the agent first and bolt on logging afterward. BoardProof goes the other way. The **first thing** we built was the receipt format. **Then** we built the runtime around it. Why?
...
- `boardproof-receipt` crate — Rust types for receipts, plus crypto sign/verify behind a feature flag.
- `boardproof-verify` — a tiny standalone binary (~720 KB stripped) that takes a receipt JSON file and reports whether it verifies. Has no internet, no database, no dependencies on anything else.
...
- ✅ **`boardproof-host` crate** — an HTTP server that serves any receipt at `/receipts/<hash>`. Step 9. See [steps/09-public-url-hosting.md](steps/09-public-url-hosting.md).
...
- 🔜 A real, running instance at `https://boardproof.dev/receipts/...`.
...
- ✅ **Step 19a — `key_id` field on receipts.** Adds an optional `body.key_id: Option<String>` to `ReceiptBody` (RFC-0001 rev **2.1**; wire `schema_version` stays `2`, additive optional field via `skip_serializing_if`). The kernel mints with the value when `Signer::key_id()` returns `Some(...)`; `Ed25519Signer` gains `.with_key_id(impl Into<String>)`. CLI flag `--key-id <string>` on `boardproof-host`. All three verifiers (Rust, TS, Python) surface the field — both in their `VerifyResult` and in the client `Decision` shapes. **Two new fixture vectors** (`with-key-id-prod-2026`, `tool-execution-with-key-id-hsm-3`) added to `canonical-v2.json`; all three implementations produce byte-identical canonical bytes and BLAKE3 content_ids. **23 new tests across three suites; workspace at 559 passing tests** at the time of merge. First post-multi-language schema-additive change — validates the conformance machinery against a real evolution. Pre-19a receipts re-canonicalize byte-identically because `None` is omitted from the canonical bytes. **Closes deep-strategy risk #3 (key-management gap)** at the receipt-format layer; the operational key directory + rotation/revocation are queued for follow-ups. See [steps/19a-key-id-field.md](steps/19a-key-id-field.md).
- ✅ **Step 27 — Publish-ready packaging for the three client libraries** (this PR). Operations bridge that makes threshold 1 *literal*. Root `package.json` with npm workspaces; `@boardproof/client` dep changed from `file:../verifier-ts` to `^0.1.0` (workspaces resolves locally for dev, registry version on publish). `publishConfig.access = "public"` + `prepublishOnly: build && test` on both TS packages. Real LICENSE-MIT + LICENSE-APACHE files copied into all three packages (Python sdist + wheel auto-pick them via setuptools 68+). Step doc `docs/steps/27-publish-clients.md` contains the full operator runbook for `npm publish` and `python -m twine upload`. Zero runtime code changes (Rust + TS source + Python source bit-for-bit identical to step 26). Same 605-test total. Side-fix: `packages/client-ts/tests/bench.mjs` had been broken since step 25's auth requirement; restored. **The actual registry pushes are operator-only** (the user holds the npm + PyPI credentials); this PR makes them safe to do. See [steps/27-publish-clients.md](steps/27-publish-clients.md).
- ✅ **Step 26 — Persistence in proposal mode**. Allows `boardproof-host --constitution <path> --db <path>` together — proposal mode + persistent `SqliteReceiptLog` instead of in-memory only. `ApiState` becomes generic over `L: ReceiptLog + Send + Sync + 'static`; handler signatures gain `<L>`. New `api::build_kernel_from_log` helper rebuilds `KernelState { sequence: last.sequence + 1, prev_hash: last.leaf_hash }` from `log.last()` so a restart-with-populated-DB continues the chain via `Kernel::resume` instead of starting at genesis (which would `AppendError::OutOfOrder`). The binary's `peek_issuer` pre-check rejects mismatched signing keys at startup with a helpful error (belt-and-suspenders + SQLite's own open-time validation). **No wire-format change** — clients run unchanged. 4 new Rust integration tests cover persistence, chain continuation across reopen, full tool-execution persistence, and issuer-mismatch rejection. Cross-language smoke (TS 52/52 + Py 84/84) confirms zero regression. Bench: SQLite ~7 ms/req, in-mem ~13 ms/req (same band; choice of backend is a durability decision, not a performance one). **First production-deployable state** for BoardProof: restart-safe + auth-gated + key-identified + persistent. See [steps/26-proposal-mode-persistence.md](steps/26-proposal-mode-persistence.md).
...
**Why it matters:** this is where BoardProof can finally call HTTP, run code, edit files — but with capability budgets enforced *and* with secrets that the model never sees in plaintext.
...
**Why it matters:** real-world agentic workflows involve agents calling other agents. Today that's a security disaster. With BoardProof's budget algebra and signed inter-agent receipts, it stops being one.
...
Phase 3.5 🚧 hardening + adoption foundations (steps 19 + 19a + 20 + 20a + 21 + 22 + 23 + 24 + 25 + 26 + 27 landed; step 19b (witness sigs + chain checkpoint receipts) + key directory + first-claw integration queued)  ← you are here
Phase 3 ⬜ planned
Phase 4 ⬜ planned
Phase 5 ⬜ planned
Phase 6 ⬜ planned
