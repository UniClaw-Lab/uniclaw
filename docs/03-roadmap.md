...
- ✅ **Step 14 — Native HTTP Fetch Tool with Capability Enforcement** (this PR). New `uniclaw-tools-http` crate: `HttpFetchTool` (synchronous GET via `ureq`), capability-allowlist gate, SSRF refusal of private/loopback/link-local/multicast IPs, bounded response, no auto-redirects. Validates the `Capability` enum + trait surface from step 13 against real network code. Adds `Capability::is_granted_by` helper. See [steps/14-http-fetch-tool.md](steps/14-http-fetch-tool.md). **Reordered from the original plan**: this step was supposed to be WASM, but WASM-with-I/O needs capability enforcement, and capability enforcement needs a real consumer to validate against. Native HTTP first → secrets next → WASM with both already proven.
- 🔜 **Step 15 — Secret Broker.** Credential injection at execution time (URLs, headers), with **fail-closed on missing required credentials** (adopted from IronClaw). Tools declare required secrets in their manifest; the broker fills them at call time; the model never sees them.
- 🔜 **Step 16 — WASM Tool Runtime.** New `uniclaw-tools-wasm` crate using `wasmtime` + WIT Component Model (adopted from IronClaw). The substrate swap: `WasmTool` adapter implements the same `Tool` trait already proven by `HttpFetchTool`, applying fuel + memory + timeout limits via `wasmtime::ResourceLimiter`.
...
Phase 3 🚧 in progress (steps 13 + 14 landed)             ← you are here
Phase 3 ⬜ planned
Phase 4 ⬜ planned
Phase 5 ⬜ planned
Phase 6 ⬜ planned
